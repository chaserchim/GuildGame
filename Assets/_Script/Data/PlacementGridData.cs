...
public class PlacementGridData
{
    ...
    /// <summary>
    /// For the new rotation system we will calculate the size in such a way that we 
    /// always check the object starting at where the mouse coursor is and checking the 
    /// cells in the diection based on the rotation
    /// </summary>
    ...
    internal bool IsSpaceOccupiedByEdgeObject(Vector3Int currentTilePosition, Vector2Int objectSize, int rotation, bool edgePlacement)
    {
        ...
        {
            Vector3Int modifiedPosition = cellPosition;
            if (rotation == 270)
            {
                modifiedPosition += new Vector3Int(1, 0, 0);
            }
            else if (rotation == 90)
            {
                modifiedPosition += new Vector3Int(0, 0, 1);
            }
            ...
            if (offset.z == 0 && offset.x <= objectSize.x)
            {
                ...
                {
                    if(objectSize.y > 1)
                        edges.UnionWith(GetEdgePositions(rotation == 270 ? modifiedPosition : cellPosition, Vector2Int.one, 0));
                    else
                        edges.UnionWith(GetEdgePositions(rotation == 270 ? modifiedPosition : cellPosition + Vector3Int.right, Vector2Int.one, 270));
                }
                else
                {
                    edges.UnionWith(GetEdgePositions(rotation == 0 || rotation == 90 ? cellPosition : modifiedPosition, Vector2Int.one, 270));
                }
            }
            else if (offset.x == 0 && offset.z <= objectSize.y)
            {
                if (rotation == 180)
                {
                    if(objectSize.x > 1)
                        ...
                        edges.UnionWith(GetEdgePositions(cellPosition + new Vector3Int(1,0,1), 
                            Vector2Int.one, 180));
                }
                else
                {
                    edges.UnionWith(GetEdgePositions(rotation == 0 || rotation == 270 ? cellPosition : modifiedPosition, Vector2Int.one, 0));
                }
            }
            else
            {
                if (rotation == 180)
                {
                    modifiedPosition += new Vector3Int(1, 0, 1);
                    //gets the edge downwards
                    edges.UnionWith(GetEdgePositions(rotation == 180 || rotation == 270 ? modifiedPosition : cellPosition, Vector2Int.one, 90));
                    //Gets the edge to the left
                    edges.UnionWith(GetEdgePositions(rotation == 0 || rotation == 270 ? cellPosition : modifiedPosition, Vector2Int.one, 180));
                }
                //Gets the edge to the right
                edges.UnionWith(GetEdgePositions(rotation == 0 || rotation == 270 ? cellPosition : modifiedPosition, Vector2Int.one, 0));
                //gets the edge upwards
                edges.UnionWith(GetEdgePositions(rotation == 180 || rotation == 270  ? modifiedPosition : cellPosition , Vector2Int.one, 270));
